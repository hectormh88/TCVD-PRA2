...
```{r}
outl_age <- mean(data$age, na.rm=T) + 3 * sd(data$age, na.rm=T); outl_age
```

```{r}
outl_chol <- mean(data$chol, na.rm=T) + 3 * sd(data$chol, na.rm=T); outl_chol
```


```{r}
sum(data$age>outl_age, na.rm=T)
```

```{r}
sum(data$chol>outl_chol, na.rm=T)
```


```{r}
hist(data$age, main="Histograma variable 'age' (Edad)",
     xlab="Edad",
     ylab="Frecuencia",
     border="blue",
     col="white",
     xlim = c(min(data$age), outl_age+10))
abline(v = outl_age, col = "red", lwd = 2)
```

```{r}
boxplot(data$age, main="Diagrama de caja variable 'age' (Edad)", xlab="Edad")
```


```{r}
hist(data$chol, main="Histograma variable 'chol' (Colesterol)",
     xlab="Colesterol",
     ylab="Frecuencia",
     border="blue",
     col="white")
abline(v = outl_chol, col = "red", lwd = 2)
```

```{r}
boxplot(data$chol, main="Diagrama de caja variable 'chol' (Colesterol)", xlab="Colesterol")
```


En nuestra exploración inicial de los datos, encontramos un valor extremadamente alto en las mediciones de colesterol: un valor de 564 mg/dL. Si bien es técnicamente posible que una persona tenga un nivel de colesterol tan alto, es muy raro y generalmente se asocia con condiciones médicas específicas como la hipercolesterolemia familiar.
